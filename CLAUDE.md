...
run.sh              → Image generation script using OpenAI DALL-E API
.env                → OPENAI_API_KEY (not committed)
```

All web apps use vanilla HTML/CSS/JS with no frameworks, no build tools, and no package manager. The apps share a glassmorphism UI style and randomly select a background image from `docs/images/`.
...
bash run.sh
```
Requires `curl`, `jq`, and `OPENAI_API_KEY` in `.env`. Outputs to `docs/images/output_YYYYMMDD_HHMMSS.jpg`.
